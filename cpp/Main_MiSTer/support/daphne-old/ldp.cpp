...
bool ldp::pre_search(const char* pszFrame, bool block_until_search_finishes)
{
	...
	{
		printf("LDP : tried to search without checking for search result first! that's bad!");
//		printf(frame);
	...
	{
		printf("LDP NOTE: ignoring seek because we're already on that frame");
...
//	}
//	else
	printf(s1, "Search to %d received", frame_number);
...
//		make_delay(get_search_latency());
		printf("WARNING : search latency needs to be redesigned, it is currently disabled");
	}
...
//				{
//					printf("LDP : blocking search didn't succeed");
//					result = false;
//				}
//			} // end if we were doing a blocking styled search
		...
		{
			printf("LDP : search failed immediately");
			m_status = LDP_ERROR;
		}
...
bool ldp::pre_skip_forward(uint16_t frames_to_skip)
{
	...
	if (m_status == LDP_PLAYING)
	{
		...
		//printline(s);
		printf("LDP: skip success");
	}
	else
	{
		printf("LDP ERROR: Skip forward command was called when the disc wasn't playing");
	}
...
bool ldp::pre_skip_backward(uint16_t frames_to_skip)
{
	...
	if (m_status == LDP_PLAYING)
	{
		...
		//printline(s);
		printf("LDP: Skipped backward");
	}
	else
	{
		printf("LDP ERROR: Skip backward command was called when the disc wasn't playing");
	}
...
void ldp::pre_step_forward()
{
	...
	if (new_frame < ((uint16_t) -1))
	{
		...
		printf("LDP : Stepping forward one frame");
		// TODO pre_search?
		//g_ldp->pre_search(frame, true);
	}
	else
	{
		printf("LDP: pre_step_forward failed bounds check");
	}
}
...
void ldp::pre_step_backward()
{
	...
	printf("LDP : Stepping backward one frame");
	// TODO pre_search?
	//g_ldp->pre_search(frame, true);
}
...
void ldp::pre_play()
{
	...
	if (m_status == LDP_SEARCHING)
	{
		printf("LDP : tried to play without checking to see if we were still seeking! that's bad!");
	...
	if (m_status != LDP_PLAYING)
	{
		...
		m_uMsFrameBoundary = 1000000 / 23.976;	// how many ms must elapse before the first frame ends, 2nd frame begins
	...
	{
		printf("LDP : disc is already playing, play command ignored");
	}

	printf("Play");	// moved to the end of the function so as to not cause lag before play command could be issued
}
...
void ldp::pre_pause()
{
	...
	{
		printf("LDP : Received pause while disc was not playing, ignoring");
	}
}
...
bool ldp::pre_change_speed(unsigned int uNumerator, unsigned int uDenominator)
{
	...
	if (uDenominator == 1)
	{
		...
		{
			m_uFramesToSkipPerFrame = 0;
			printf("ERROR : uNumerator of 0 sent to pre_change_speed, this isn't supported, going to 1X");
		}
	}
	// else if this is < 1X
	else if (uNumerator == 1)
	{
		...
		{
			m_uFramesToStallPerFrame = 0;
			printf("ERROR : uDenominator of 0 sent to pre_change_speed, this is undefined, going to 1X");
		}
	}
...
{
// TODO lair cpu 4000000
//	bool bEmulatedCpu = (get_cpu_hz(0) != 0);	// whether we've got an emulated cpu
	bool bEmulatedCpu = 4000000 != 0;	// whether we've got an emulated cpu

	// safety check: make sure that we're not using an emulated CPU
	if (bEmulatedCpu)
	{
		printf("think_delay() should not be used with an emulated CPU. Don't use blocking seeking maybe?");
		//set_quitflag();
	}
	...
	{
		printf("think_delay() should not be called until pre_init() has been called.");
		//set_quitflag();
	}
...
{
	printf("NOTE : Search blanking cannot be modified with this laserdisc player!");
}

// causes LDP to blank video while skipping
void ldp::set_skip_blanking(bool enabled)
{
	printf("NOTE : Skip blanking cannot be modified with this laserdisc player!");
}

void ldp::set_seek_frames_per_ms(double value)
{
	printf("NOTE : Seek delay is not supported with this laserdisc player!");
}
...
{
	printf("Audio1 enable received (ignored)");
}

void ldp::enable_audio2()
{
	printf("Audio2 enable received (ignored)");
}

void ldp::disable_audio1()
{
	printf("Audio1 disable received (ignored)");
}

void ldp::disable_audio2()
{
	printf("Audio2 disable received (ignored)");
}
